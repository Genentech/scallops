...
# SCALLOPS changed heap import and added noexcept to heap.pxd
from scallops.segmentation.heap cimport *
...
def propagate(np.ndarray[DTYPE_DOUBLE_t, ndim=2, negative_indices=False, mode='c'] image,
    ...
    cdef np.ndarray[DTYPE_INT32_t, ndim=1, negative_indices=False, mode='c'] delta_i = np.array(
        (-1, -1, -1, 0, 0, 1, 1, 1), dtype=np.int32)
    cdef np.ndarray[DTYPE_INT32_t, ndim=1, negative_indices=False, mode='c'] delta_j = np.array(
        (-1, 0, 1, -1, 1, -1, 0, 1), dtype=np.int32)
