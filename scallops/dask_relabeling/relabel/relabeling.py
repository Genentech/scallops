...
def merge_overlapped_tiles(
    labels: da.Array, overlaps: List[int], spatial_dims: int = 2
) -> da.Array:
    merged_depth = tuple(
        [0] * (labels.ndim - spatial_dims)
...
def prepare_input(
    img: da.Array, overlaps: List[int], spatial_dims: int = 2
) -> da.Array:
...
def image2labels(
    ...
    labels = remove_overlapped_labels(
        labels, overlaps=overlaps, threshold=threshold, spatial_dims=spatial_dims
    )

    labels = merge_overlapped_tiles(
        labels, overlaps=overlaps, spatial_dims=spatial_dims
    )

    # Remove the pad added with prepare_input
    labels = labels[
        tuple(
            [slice(None)] * (labels.ndim - spatial_dims)
...
def labels2geojson(
    ...
    if object_classes is None:
        classes_ids = range(
            labels.shape[: (labels.ndim - spatial_dims)][0]
            if labels.ndim - spatial_dims > 0
            else 1
        )
...
def sort_label_indices(labels: da.Array, spatial_dims: int = 2) -> da.Array:
    """Sort the indices of all labeled objects in the dask array to be an uninterrupted sequence
    atarting at 1.

    This triggers the compute over the whole image and therefore should be used only when an
    uninterrupted sequence of label indices is exctiictly necessary.

    It is recommended to pre-compute the labels before sorting the indices, either by using
    labels.persist(), or saving them into a temporary file (e.g. a zarr array) and reopening them
    again as a dask array.
    """
    classes = None
    if labels.ndim > spatial_dims:
        labels_chunks = labels.chunks
