...
# ---------------------------------------------------------------------------
# Verbatim from origin/main — test_compute_recall
# ---------------------------------------------------------------------------
...
def test_compute_recall(use_dask):
    ...
    if use_dask:
        null_distribution = da.from_array(null_distribution).rechunk((1,))
        query_distribution = da.from_array(query_distribution).rechunk((1,))
